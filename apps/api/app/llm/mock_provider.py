...
class MockLLMProvider(LLMProvider):
    ...
    def _response_for_scenario(self, scenario: str, prompt: str) -> str:
        ...
        if scenario == "practice_quiz":
            return json.dumps(
                {
                    "title": "事务、JOIN 与索引专项练习",
                    "description": "围绕数据库系统期末复习中的 JOIN、事务隔离级别、B+树索引和查询优化生成的原创测验。",
                    "difficulty": "medium",
                    "questions": [
                        {
                        ...
                        },
                        {
                            ...
                            "options": [
                                {"key": "A", "text": "叶子节点有序连接，适合范围查询"},
                                {"key": "B", "text": "树高较低，有利于减少磁盘页访问"},
                                {"key": "C", "text": "所有查询都会因为索引变快"},
                                {"key": "D", "text": "联合索引使用时要关注最左前缀"},
                            ],
                            "correct_answer": {"answers": ["A", "B", "D"]},
                            "explanation": "B+ 树适合范围扫描，但索引是否生效仍受条件和最左前缀影响。",
                        ...
                        },
                        {
                            ...
                            "options": [
                                {"key": "A", "text": "LEFT JOIN 保留左表不匹配记录"},
                                {"key": "B", "text": "INNER JOIN 不需要连接条件"},
                                {"key": "C", "text": "LEFT JOIN 一定比 INNER JOIN 快"},
                                {"key": "D", "text": "二者结果永远一致"},
                            ],
                            "correct_answer": {"answer": "A"},
                            "explanation": "LEFT JOIN 会保留左表全部记录，INNER JOIN 只保留匹配记录。",
                        ...
                        },
                        {
                            ...
                            "stem": "请用自己的话说明幻读和不可重复读的区别。",
                            "options": [],
                            "correct_answer": {"keywords": ["同一行", "范围", "结果集", "幻读"]},
                            "explanation": "答案应区分同一行值变化和范围查询结果集变化。",
                        ...
                        },
                        {
                            ...
                            "stem": "写出 student、enrollment、course 三张表关联查询学生姓名、课程名和成绩的 SQL。",
                            "options": [],
                            "correct_answer": {
                            ...
                            "explanation": "SQL 实操题检查 SELECT、JOIN、表名和连接条件。本阶段不执行学生 SQL。",
                            "knowledge_point": "JOIN",
                            "difficulty": "medium",
                            "score": 10,
                        },
                        {
                            "question_type": "short_answer",
                            "stem": "如何判断一个 SQL 查询是否适合建立索引？",
                            "options": [],
                            "correct_answer": {"keywords": ["WHERE", "选择性", "范围查询", "最左前缀", "执行计划"]},
                            "explanation": "索引判断需要结合过滤条件、选择性、范围查询和执行计划。",
        ...
        if scenario == "evaluation":
            return json.dumps(
                {
                    "feedback_summary": "本次测验用于演示 EvaluatorAgent 的错因分析和掌握度更新。",
                    "weak_points": ["事务隔离级别", "B+树索引"],
                    "strengths": ["SQL基础"],
                    "recommended_actions": [
                        "复习事务隔离级别图解讲义",
                        "完成 B+树索引实操案例",
                    ],
                    "next_plan_suggestion": "优先完成事务与索引相关的补救资源，然后进行第二次测验。",
                },
                ensure_ascii=False,
                indent=2,
            )
