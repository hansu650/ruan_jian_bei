...
SCENARIOS = [
    LLMScenario(
        ...
        sample_prompt="请用两句话说明 EduForge 当前在做什么。",
    ),
    LLMScenario(
        key="profile",
        title="学习画像草稿",
        description="返回可解析 JSON，为 ProfileAgent 提供演示输出。",
        sample_prompt="学生大二，计算机专业，7天复习数据库，SQL中等，事务和索引薄弱。",
    ),
    LLMScenario(
        key="learning_path",
        title="学习路径草稿",
        description="返回可解析 JSON，演示 PlannerAgent 的路径规划输出形态。",
        sample_prompt="请为数据库系统期末复习生成 7 天学习路径，重点补事务、JOIN 和 B+树。",
    ),
    LLMScenario(
        key="resource_note",
        title="讲义资源",
        description="返回 Markdown 讲义，用于 ResourceAgent 的 lecture_note 演示。",
        sample_prompt="请生成一份事务隔离级别速记讲义，适合大二学生。",
    ),
    LLMScenario(
        key="resource_mindmap",
        title="思维导图资源",
        description="返回 mermaid mindmap fenced code block，用于 ResourceAgent 的 mindmap 演示。",
        sample_prompt="请为 B+树索引生成一个思维导图。",
    ),
    LLMScenario(
        key="quiz",
        title="练习题资源",
        description="返回可解析 JSON 或 Markdown 题目，用于 ResourceAgent 的 quiz 演示。",
        sample_prompt="请围绕幻读和 B+树生成两道练习题。",
    ),
    LLMScenario(
        key="resource_reading",
        title="拓展阅读资源",
        description="返回阅读提纲和阅读问题，用于 ResourceAgent 的 reading 演示。",
        sample_prompt="请为查询优化生成拓展阅读建议。",
    ),
    LLMScenario(
        key="resource_practice_case",
        title="实操案例资源",
        description="返回 SQL/代码实操任务，用于 ResourceAgent 的 practice_case 演示。",
        sample_prompt="请为 SQL JOIN 生成一个实操案例。",
    ),
    LLMScenario(
        key="resource_video_script",
        title="视频脚本资源",
        description="返回短视频或动画讲解脚本，用于 ResourceAgent 的 video_script 演示。",
        sample_prompt="请为事务隔离级别生成一个 2 分钟视频脚本。",
    ),
    LLMScenario(
        key="tutor",
        title="智能答疑草稿",
